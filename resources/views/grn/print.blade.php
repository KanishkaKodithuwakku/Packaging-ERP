...
<head>
    ...
    <title>GRN - {{ $grn->formatted_grn_no }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="grn-info">
        ...
        <div class="info-section">
            ...
            <div class="info-item">
                ...
                {{ ucfirst(str_replace('_', ' ', $grn->status ?? 'pending')) }}
            </div>
            <div class="info-item">
                <span class="info-label">Receiving Status:</span>
            ...
            <div class="info-item">
                ...
                {{ $grn->processed_at->format('Y-m-d H:i') }}
            </div>
            @endif
            @if($grn->notes)
            <div class="info-item">
        ...
        <tbody>
            @forelse($grn->items as $grnItem)
                @php
                    ...
                    $qtyExpected = $grnItem->qty_expected ?? 0;
                    $qtyReceived = $grnItem->qty_received_partial ?? 0;
                    $qtyPending = $grnItem->qty_pending ?? 0;
                    $qtyProcessed = $grnItem->qty_processed ?? 0;
                    $qtyRemaining = max(0, $qtyReceived - $qtyProcessed);
                @endphp
                <tr>
                    @if($grn->isFromPurchaseOrder())
                    ...
                    <td class="text-right">{{ number_format($qtyExpected, 2) }}</td>
                    <td class="text-right"><strong>{{ number_format($qtyReceived, 2) }}</strong></td>
                    <td class="text-right">{{ number_format($qtyPending, 2) }}</td>
                    <td class="text-right">{{ number_format($qtyProcessed, 2) }}</td>
                    <td class="text-right">{{ number_format($qtyRemaining, 2) }}</td>
                    <td>{{ $grnItem->uom }}</td>
                    <td>
                        @if($grnItem->is_fully_received ?? false)
                            Fully Received
            ...
            @empty
                <tr>
                    <td colspan="{{ $grn->isFromPurchaseOrder() ? '11' : '9' }}" class="text-center">No items</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="summary-section">
        <div class="summary-item">
            ...
            <span class="summary-value">{{ number_format($totalExpected, 2) }} PCS</span>
        </div>
        <div class="summary-item">
            <span class="summary-label">Total Received:</span>
            <span class="summary-value" style="color: #2563EB;">{{ number_format($totalReceived, 2) }} PCS</span>
        </div>
        <div class="summary-item">
            <span class="summary-label">Total Processed:</span>
            <span class="summary-value" style="color: #059669;">{{ number_format($totalProcessed, 2) }} PCS</span>
        </div>
        <div class="summary-item">
            <span class="summary-label">Total Pending:</span>
            <span class="summary-value" style="color: #d97706;">{{ number_format($totalPending, 2) }} PCS</span>
        </div>
        @if($grn->total_value)
        <div class="summary-item">
            <span class="summary-label">Total Value:</span>
            <span class="summary-value">{{ $grn->getCurrencySymbol() }}{{ number_format($grn->total_value, 2) }}</span>
        </div>
        @endif
    </div>
    ...
    <script>
        ...
        window.onload = function() {
            ...
            document.title = 'GRN - {{ $grn->formatted_grn_no }}';
