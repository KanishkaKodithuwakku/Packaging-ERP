...
class LedgerEntriesReport extends Component
{
    ...
    public function generateLedgerEntries()
    {
        if (!$this->selectedLedgerId) {
        ...
        $this->subtitle = "Ledger entries for [{$this->ledger->code}] {$this->ledger->name} from {$startDate} to {$endDate}";
    ...
    private function loadEntries()
    {
        ...
        // Order by date DESCENDING (newest first) - this is the key difference from Ledger Statement
        $query->whereHas('entry', function ($q) {
            $q->orderBy('date', 'desc');
        });
    ...
    {
        return view('livewire.reports.ledger-entries-report');
    }
}
